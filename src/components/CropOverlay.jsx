...
/**
 * Interactive crop selector. Renders a dimmed mask over the canvas with a
 * draggable/resizable rectangle. The crop value is normalized in source-pixel
 * space (0..1) — independent of the canvas's display size.
 */
export default function CropOverlay({
  ...
  // Normalized to canvas (0..1) — the inset rect where the video is drawn.
  const baseN = useMemo(() => ({
  ...
  const active = crop || { x: 0, y: 0, width: 1, height: 1 };

  // Crop rect in canvas-normalized coords (for placing the DOM rectangle).
  const rectN = useMemo(() => ({
  ...
  const pointerToSource = useCallback((e) => {
  ...
  const onMove = useCallback((e) => {
    ...
    if (drag.mode === 'move') {
      ...
      // Resize: edges of the box. Compute new left/right/top/bottom.
      let l = o.x, r = o.x + o.width;
      let t = o.y, b = o.y + o.height;
