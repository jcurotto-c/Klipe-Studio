...
/**
 * Side panel for editing a single zoom segment. The inputs are local-edits
 * onto the segment shape from zoom-engine; we forward changes upward as a
 * patch object so EditorView owns the immutable update.
 */
export default function ZoomInspector({
  ...
  const setScale = (v) => onChange({ scale: Number(v) });
  ...
  const setEase = (key, v) => onChange({ [key]: Math.max(0, Number(v)) });
