...
import { PREVIEW_PADDING_SCALE } from '../lib/layout';
import { sampleBlurRegion } from '../lib/blur-engine';
import type { BlurRegion, BlurSampleRect, Crop } from '../types';
...
export default function BlurOverlay({
  ...
  // Aspect-mismatched (fillFrame): source is cover-fit, so it overflows the
  // wrapper on the long axis with `paddingScale` ignored — same as the
  // renderer's fillFrame branch.
  const inset = useMemo<Inset>(() => {
    const sourceAspect = sourceWidth / sourceHeight;
    const targetAspect =
      aspectRatio && isFinite(aspectRatio) && aspectRatio > 0 ? aspectRatio : sourceAspect;
    const fillFrame = Math.abs(targetAspect - sourceAspect) > 0.005;
    if (fillFrame) {
  ...
  }, [sourceWidth, sourceHeight, aspectRatio, paddingScale]);
