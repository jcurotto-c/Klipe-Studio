...
const SHAPE_META: Record<'pointer' | 'bone' | 'hand', CursorShapeMeta> = {
  ...
  hand:    { url: handSvgUrl,    width: 767.314286, height: 746,  hotspotXRatio: 322 / 767.314286, hotspotYRatio: 37 / 746 },
};
...
(Object.keys(images) as ShapeKey[]).forEach((key) => {
  ...
  switch (key) {
    ...
    // The hand PNG inside this SVG has noticeable transparent margin around
    // the actual hand artwork, so a 40px container ends up looking smaller
    // than the 28px pointer. 56px makes the visible hand match the pointer.
    case 'hand': img.style.width = '56px'; break;
    default: break;
  }
...
// Map the OS-detected cursor type to one of our SVG slots. Anything that
// isn't text or pointer falls back to the pointer SVG (matches the editor's
// default render when no override applies).
